...
--FOR GETTING PERSONS KNOWING A PERSON:
--Select UNNEST(persons) FROM isKnownByPersons WHERE pid = 1001;
--will this be a set union since we also need to return the Persons pid

--FOR GETTING AI persons:
--SELECT UNNEST(persons) FROM skillOfPersons WHERE skill='AI';

--FOR GETTING PROGRAMMING AND NETWORKS persons - note to self: USE () WHEN THINGS DON'T WORK


/*SELECT set_union((SELECT persons FROM skillOfPersons WHERE skill = 'Networks'), 
		 (SELECT persons FROM skillOfPersons WHERE skill = 'Programming'));
*/
--this query is saying that the people know person who has AI as a skill, but they do not have networks or programming

SELECT p.pid, p.pname, set_difference((SELECT set_intersection((SELECT persons FROM isKnownByPersons WHERE pid=p.pid),
								 (SELECT persons FROM skillOfPersons WHERE skill='AI'))),
				        (SELECT set_intersection((SELECT persons FROM skillOfPersons WHERE skill = 'Networks'), 
...
SELECT DISTINCT p2.pid, p2.pname
FROM Person p2
WHERE superset((SELECT s2.skills FROM personHasSkills s2 WHERE p2.pid = s2.pid), 
		(SELECT s.skills
		 FROM personHasSkills s, companyHasEmployees e, worksFor w, Person p 
		 WHERE s.pid = p.pid AND e.cname='Google' AND is_in(p.pid, e.employees)
			AND w.pid = p.pid AND w.salary <= ALL(SELECT salary FROM worksFor WHERE cname='Google'))); 


\qecho 'Problem 3'
-- Find the following set of sets {S | S ⊆ Skill ∧ |S| ≤ 2}.
...
FROM	Skill
UNION
SELECT	ARRAY[]::TEXT[] AS skills
UNION
SELECT	DISTINCT make_pair(s.skill, s2.skill) as skills
FROM	Skill s, Skill s2
WHERE	s.skill <> s2.skill;


/* couldn't get set_union to work so I just used regular unions instead
SELECT DISTINCT set_union(set_union((SELECT DISTINCT make_singleton(s.skill) AS skills),(SELECT ARRAY[]::TEXT[] AS skills)),
			  (SELECT DISTINCT make_pair(s.skill, s2.skill) AS skills)) 
FROM Skill s, Skill s2
WHERE s.skill <> s2.skill;
*/
...
\qecho 'Problem 6'
-- Starting from the courseGrades view in Problem (5) solve the following
-- queries:
...
FROM	courseGrades c
WHERE	cardinality(c.gradeInfo)=1;

DROP VIEW courseGrades;
DROP VIEW e;
...
CREATE VIEW f AS 
 SELECT e.cno, e.grade, array_to_json(array_agg(json_build_object('sid',e.sid))) as students
 FROM	Enroll e
 GROUP BY (e.cno, e.grade)
 ORDER BY (e.cno, e.grade);

CREATE VIEW jcourseGrades AS
 SELECT json_build_object('cno', cno, 'gradeInfo', 
 	array_to_json(array_agg(json_build_object('grade', grade, 'students', students)))) as gradeInfo
 FROM	f
 GROUP BY(cno)
 ORDER BY(cno);
